...
;(function () {
  ...
  async function storeApi(path, opts = {}) {
    ...
    const res = await fetch('/api/store' + path, { ...opts, headers })
  ...
  async function renderBanners() {
    ...
    el.innerHTML = `
      <div class="status-line"><b>${slides.length}</b> banner slide${slides.length === 1 ? '' : 's'} — uploaded slides go <b>live</b> on the homepage hero carousel and link to the shop</div>
      <div class="toolbar">
    ...
    const paint = () => {
      ...
      grid.innerHTML = slides
        ...
        <div class="banner-card panel">
          <div class="banner-img"><img src="${s.image}" alt="Banner slide ${i + 1}" /></div>
          <div class="banner-foot">
            <span class="tag ${s.active !== false ? 'stock-in' : 'low'}">${s.active !== false ? 'Live' : 'Paused'}</span>
            <span class="banner-pos">Slide ${i + 1}</span>
    ...
    $('#banner-file', el).addEventListener('change', async (e) => {
      ...
      try {
        for (const file of files) {
          slides.push({ image: await readBannerImage(file), active: true })
        }
        slides = await persist(slides, 'Banner uploaded — live on homepage ✓')
        renderBanners()
      } catch (err) {
